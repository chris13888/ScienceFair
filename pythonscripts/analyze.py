...
class Parser():
    ...
    def scale(self, info):
        ...
        num = (self.worddict[info[0]] / self.wordcount) * ((1/info[1])**500)
        if info[2] != '----':
...
correct = {i: 0 for i in range(1, 32)}
correct[False] = 0
total_time = 0
number_of_cases = 0
tester = Parser(depth=2)
with open('testdata.txt', 'r') as file:
    while True:
        try:
            misspelled, correction = file.readline().split()
        except:
            break

        start = time.clock()
        corrections = tester.correct(misspelled, '----')
        end = time.clock()
        ...
        for index, value in enumerate(corrections):
            ...
            if value == correction:
                found = index
        if found:
            print(f'The right correction was not found for word {misspelled} (the correct spelling was {correction}).')
            print(tester.correct(misspelled, '----'))
        correct[found] += 1

print(f'Test cases: {number_of_cases}\nTotal time: {total_time}\nAverage time: {total_time/number_of_cases}')
print(correct)
